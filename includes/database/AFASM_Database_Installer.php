...
class AFASM_Database_Installer {
	...
	public function create_secret_key_if_is_not_defined() {
		...
		if ( ! defined( 'AFASM_ACCESS_TOKEN_SECRET_KEY' ) || empty( AFASM_ACCESS_TOKEN_SECRET_KEY ) ) {
			add_option( 'AFASM_ACCESS_TOKEN_SECRET_KEY', base64_encode( openssl_random_pseudo_bytes( 30 ) ), '', false ); // phpcs:ignore
		}

		if ( ! defined( 'AFASM_REFRESH_TOKEN_SECRET_KEY' ) || empty( AFASM_REFRESH_TOKEN_SECRET_KEY ) ) {
			add_option( 'AFASM_REFRESH_TOKEN_SECRET_KEY', base64_encode( openssl_random_pseudo_bytes( 30 ) ), '', false ); // phpcs:ignore
		}
